...
``` html
{% raw %}{% load django_bootstrap5 %}{% endraw %}
<!DOCTYPE html>
<html lang="ko">
  ...
  <body>
    <div class="container">
      {% raw %}{% block body %}{% endblock body %}{% endraw %}
    </div>
    {% raw %}{% bootstrap_javascript %}{% endraw %}
  </body>
...
{% block body %} 
  ...
  {% raw %}<form action="{% url ' todo:create' %}">{% endraw %}
...
<body>
	<h1>글쓰기</h1>
  <!-- method를 POST로 변경 -->
  {% raw %}<form action="{% url 'articles:create' %}" method="POST">{% endraw %}
    {% csrf_token %}
...
<body>
	<h1>글쓰기</h1>
  <!-- method를 POST로 변경 -->
  {% raw %}<form action="{% url 'articles:create' %}" method="POST">
    {% csrf_token %}{% endraw %}
    <!-- 기존의 form을 대체할 수 있다 -->
    {{ article_form.as_p }}
</body>
...
<body>
  ...
  <p>{{ article.content }}</p>
  {% raw %}<a href="{% url 'article:update' article.pk %}">수정하기</a>{% endraw %}
</body>
...
<body>
	<h1>글 수정하기</h1>
  {% raw %}<form action="{% url 'articles:update' %}" method="POST"></form>{% endraw %}
  {{ article_form.as_p }}
